  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (data.success) {
          setProducts(data.products.filter(p => p.category === "hoodies"));
        }
      } catch (error) {
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Buy premium coding-themed Hoodies from CodesWear.com" />
      </Head>
        ...
        <div className="container px-5 py-16 mx-auto">
          ...
          <div className="flex flex-col text-center w-full mb-12">
            ...
            <p className="lg:w-2/3 mx-auto leading-relaxed text-base">
              Explore our exclusive range of coding-themed hoodies. High quality, warm, and perfect for developers.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-wrap -m-4">
              {products.length > 0 ? (
                          ...
                          {product.sizes.map(size => (
                            <span
                              ...
                              className={`px-2 py-1 text-xs border rounded cursor-pointer ${
                                product.availableQty === 0 ? "opacity-50 cursor-not-allowed" : "bg-gray-100"
                              }`}
                              title={`Size ${size}`}
                            >
                              {size}
                          ...
                          {product.colors.map(color => (
                            <span
                              ...
                              className={`w-5 h-5 rounded-full border cursor-pointer ${
                                product.availableQty === 0 ? "opacity-50 cursor-not-allowed" : ""
                              }`}
                ...
                <p className="text-center w-full text-gray-500">No hoodies available right now.</p>
              )}
            </div>
          )}
        </div>
